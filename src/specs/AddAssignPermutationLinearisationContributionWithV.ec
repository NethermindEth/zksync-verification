...
module AddAssignPermutationLinearisationContributionWithV = {
  ...
  proc mid(dest : uint256, stateOpening0AtZ : uint256, stateOpening1AtZ : uint256, stateOpening2AtZ : uint256, stateOpening3AtZ : uint256, factor: int, state_beta: int, state_z: int, state_gamma: int): unit = {
    var factor, _1, _3, gamma, _4, intermediateValue, _6, _7, _9, _10, _12, _13, l0AtZ, _16, _17, _18, _20, _21, _23, beta', gamma_1, _25, _26, _27, intermediateValue_1, _29, _30, _31, _33, _34, _35, _36;
    factor <- ((
      factor *
    ...
    _20 <@ Primops.mload(STATE_BETA_SLOT);
    _21 <@ Primops.mload(STATE_POWER_OF_ALPHA_4_SLOT);
    factor <- (PurePrimops.mulmod _21 _20 Constants.R);
    _23 <@ Primops.mload(PROOF_COPY_PERMUTATION_GRAND_PRODUCT_OPENING_AT_Z_OMEGA_SLOT);
    factor <- (PurePrimops.mulmod factor _23 Constants.R);
    beta' <@ Primops.mload(STATE_BETA_SLOT);
    gamma_1 <@ Primops.mload(STATE_GAMMA_SLOT);
    _25 <@ Primops.mload(PROOF_COPY_PERMUTATION_POLYS_0_OPENING_AT_Z_SLOT);
    _26 <- (PurePrimops.mulmod _25 beta' Constants.R);
    _27 <- (PurePrimops.addmod _26 gamma_1 Constants.R);
    intermediateValue_1 <- (PurePrimops.addmod _27 stateOpening0AtZ Constants.R);
    factor <- (PurePrimops.mulmod factor intermediateValue_1 Constants.R);
    _29 <@ Primops.mload(PROOF_COPY_PERMUTATION_POLYS_1_OPENING_AT_Z_SLOT);
    _30 <- (PurePrimops.mulmod _29 beta' Constants.R);
    _31 <- (PurePrimops.addmod _30 gamma_1 Constants.R);
    intermediateValue_1 <- (PurePrimops.addmod _31 stateOpening1AtZ Constants.R);
    factor <- (PurePrimops.mulmod factor intermediateValue_1 Constants.R);
    _33 <@ Primops.mload(PROOF_COPY_PERMUTATION_POLYS_2_OPENING_AT_Z_SLOT);
    _34 <- (PurePrimops.mulmod _33 beta' Constants.R);
    _35 <- (PurePrimops.addmod _34 gamma_1 Constants.R);
    intermediateValue_1 <- (PurePrimops.addmod _35 stateOpening2AtZ Constants.R);
    factor <- (PurePrimops.mulmod factor intermediateValue_1 Constants.R);
    _36 <@ Primops.mload(STATE_V_SLOT);
    factor <- (PurePrimops.mulmod factor _36 Constants.R);
    PointMulIntoDest.low(VK_PERMUTATION_3_X_SLOT, factor, QUERIES_BUFFER_POINT_SLOT);
    PointSubAssign.low(dest, QUERIES_BUFFER_POINT_SLOT);
  }
}.
