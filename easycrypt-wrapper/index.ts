...
// function runEasycrypt(file: string, outerErrors: EasycryptError[], outerSuccesses: string[]) {
// 	// An inner call is made only if errors are generated by the first run
// 	const outerCall = (outerErrors.length === 0);
...
// 					printSuccesses(successes);
// 				}
// 			} else if (outerCall && errors.length > 0) { // Don't output yet because inner call is async
// 				runEasycrypt(tempFile, errors, successes);
// 			} else {
// 				if (TERMINAL_MODE) { // Reverse order of outputs depending on output format
// 					printSuccesses(outerSuccesses);
// 					printGoals(goals);
